...
public:
    ...
    void imageCallback(const sensor_msgs::ImageConstPtr &msg) {
        try {
            ...
            //ROS_INFO("After Inference: ==================== %s", cuda_post_process_);
		...
		//ROS_INFO("After Inference: ====================  GPU post-processing would go here");
...
private:
    ...
    //ROS_INFO("process_mask: ==================== %d", dets.size());
    ...
    //cv::parallel_for_(cv::Range(0, dets.size()), [&](const cv::Range& range) {
		...
		//std::cout<<"++++++++++++++++++++++++++, " << r << ", " << kInputW <<", " << kInputH << std::endl;
		...
		/*cv::Mat mask_8u;
                mask_mat.convertTo(mask_8u, CV_8U, 255.0);
		std::string gray_mask_path = "/home/unitree/piper_ws/mask_gray.png";
                cv::imwrite(gray_mask_path, mask_8u);
                ROS_INFO("灰度掩码已保存至：%s", gray_mask_path.c_str());
		...
		/*} catch (const cv::Exception& e) {
			ROS_ERROR("OpenCV 4.5.4 cv::resize异常：%s，错误位置：%s（第%d行）",
              			e.what(), e.file.c_str(), e.line);
		}

		cv::Mat resized_mask_8u;
                masks[i].convertTo(resized_mask_8u, CV_8U, 255.0);
                gray_mask_path = "/home/unitree/piper_ws/resized_mask_gray.png";
                cv::imwrite(gray_mask_path, resized_mask_8u);
                ROS_INFO("resized 灰度掩码已保存至：%s", gray_mask_path.c_str());
