...
class SO101Follower(Robot):
    ...
    def __init__(self, config: SO101FollowerConfig):
        ...
        if config.port is None:
            raise ValueError(
                "The serial port for the robot arm is not specified. Please provide it by appending `--robot.port=/dev/ttyXYZ` to your command, replacing `/dev/ttyXYZ` with the correct device path."
            )
