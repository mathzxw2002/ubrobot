    ...
    while True:
        ...
        if odom_infer is not None and rgb_bytes is not None and depth_bytes is not None:
            ...
            if 'trajectory' in response:
                ...
                if actions:
                    try:
                        # Send the action to the teleoperate script's server
                        requests.post('http://127.0.0.1:8001/action', json={'action': actions[0]}, timeout=0.1)
                    except requests.RequestException as e:
                        # Log error if the other script is not running or fails
                        print(f"Could not send action to teleop server: {e}")
...
class Go2Manager(Node):
    def __init__(self):
        ...
        rgb_down_sub = Subscriber(self, Image, "/ascamera_hp60c/camera_publisher/rgb0/image")
        depth_down_sub = Subscriber(self, Image, "/ascamera_hp60c/camera_publisher/depth0/image_raw")
        ...
        self.odom_sub = self.create_subscription(Odometry, "/RGBD/Odometry", self.odom_callback, qos_profile)
        # publisher
