...
class SO101Client(Robot):
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                f"{self} is already connected. Do not run `robot.connect()` twice."
            )
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        and a camera frame. Receives over ZMQ.
        """
        if not self._is_connected:
            raise DeviceNotConnectedError(f"{self} is not connected. You need to run `robot.connect()`.")
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command SO101 to move to a target joint configuration. Sends over ZMQ
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                f"{self} is not connected. You need to run `robot.connect()` before disconnecting."
            )
